...
from modules.file import (
    ...
    format_file_history,
    get_file_history as get_file_history_impl,
    get_open_files,
    hash_content,
    track_file_change,
)
...
from modules.memory_utils import _set_memory, _search_memories, _delete_memory
...
@pytest.fixture
def mock_session_id():
    """Mock session ID for memory operations."""
    return "test-session-new"
...
def mock_memory_utils():
    ...
    mock = MagicMock()
    mock._set_memory = MagicMock(return_value=True)
    mock._search_memories = MagicMock(return_value=[])
    mock._delete_memory = MagicMock(return_value=True)
    return mock
...
class TestFileEditor:
    ...
    def test_file_editor_init(self, mock_memory_utils):
        ...
        editor = FileEditor(memory_utils_module=mock_memory_utils)
        
        assert editor._memory_utils is mock_memory_utils
        assert editor._set_memory is mock_memory_utils._set_memory
        assert editor._search_memories is mock_memory_utils._search_memories
        assert editor._delete_memory is mock_memory_utils._delete_memory

    def test_file_editor_init_default(self):
        """Test FileEditor with default (lazy) imports."""
        editor = FileEditor()
        assert editor._set_memory is None
        assert editor._search_memories is None
        assert editor._delete_memory is None
    ...
    async def test_open_file_success(self, temp_file, mock_session_id, mock_memory_utils):
        ...
        assert mock_memory_utils._set_memory.called
...
class TestMemoryFunctions:
    ...
    def test_track_file_change(self, mock_session_id):
        ...
        with patch('modules.file.memory._get_set_memory') as mock_get:
            mock_set = MagicMock()
...
class TestMemoryEdgeCases:
    ...
    def test_format_file_history_malformed_data(self):
        ...
        memories = [
            {"properties": {}},  # Missing fields
            {"properties": {"path": "/test.py", "success": "true"}},  # Partial
        ]
    ...
    def test_get_open_files_with_files(self, mock_session_id):
        """Test get_open_files with files open."""
        with patch('modules.memory_utils._search_memories') as mock:
            mock.return_value = [
                {"properties": {"path": "/test.py"}}
            ]
            result = get_open_files(mock_session_id)
            assert len(result) == 1
    ...
    def test_list_open_files_method(self, mock_session_id):
        ...
        with patch('modules.file.editor.get_open_files') as mock_get:
            mock_get.return_value = [
                {"properties": {"path": "/test.py", "line_start": "0", "line_end": "*"}}
            ]
